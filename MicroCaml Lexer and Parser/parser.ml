...
| Some (Tok_Int int) -> let expr = Value (Int (match toks with
    ...
    | a::b -> ((match a with
      | Tok_Int id -> id
      | _ -> raise (InvalidInputException "There is no token"))))) in
       let token = match_token toks (Tok_Int int) in (token, expr)            
| Some (Tok_Bool bool) -> let expr = Value (Bool (match toks with
    | [] -> raise (InvalidInputException "There is no token")
    | a::b -> ((match a with
      | Tok_Bool t -> t
      | _ -> raise (InvalidInputException "no match"))))) in 
      let token = match_token toks (Tok_Bool bool) in (token, expr)
