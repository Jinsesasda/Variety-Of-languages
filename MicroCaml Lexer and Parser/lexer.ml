...
let rec tok s pos =
  if pos >= String.length s then []
    ...
    if (Str.string_match re_positiveInt s pos)
    ...
    else if (Str.string_match re_bool s pos) then let token = Str.matched_string s in (Tok_Bool (bool_of_string token))::(tok s (Str.match_end()))

    else if (Str.string_match re_id s pos) then let token = Str.matched_string s in (Tok_ID token)::(tok s (Str.match_end()))

    else if (Str.string_match re_string s pos) then let token = Str.matched_string s in (Tok_String (String.sub token 1 ((String.length token)-2)))::(tok s (Str.match_end()))
