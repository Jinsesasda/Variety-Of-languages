...
let character (nfa: (int, char) nfa_t) ch=
  let start = fresh() in 
  let conclusion = fresh() in 
  {
    sigma= [ch]; 
    qs= [start; conclusion]; 
    q0= start; 
    fs= [conclusion]; 
    delta= [(start, Some ch, conclusion)];
  }

let unionn (nfa1: (int, char) nfa_t) (nfa2: (int, char) nfa_t) =
let start1 = fresh() in let conclusion = fresh() in
{
  sigma= (union (nfa1.sigma) (nfa2.sigma)); 
  qs= union (union nfa1.qs nfa2.qs) ([start1; conclusion]) ; 
  q0= start1;
  fs= [conclusion]; 
  delta= ((union nfa1.delta nfa2.delta) @ ([(start1, None, nfa1.q0); (start1, None, nfa2.q0)] @ ((trans_help nfa1.fs conclusion) @ (trans_help nfa2.fs conclusion))) );
}
...
{
  sigma= nfa1.sigma; 
  qs= union nfa1.qs [start1; conclusion];
  q0 = start1;
  fs = [conclusion];
  delta= union nfa1.delta ((trans_help nfa1.fs conclusion) @[(start1, None, nfa1.q0); (start1, None, conclusion); (conclusion, None, start1)]);
}
...
| Union (a, b) -> let something = (helper_regex b nfa1) in unionn (helper_regex a something) something
| Concat (a, b) -> let something = (helper_regex b nfa1) in concat (helper_regex a something) something
| Star a -> closure (helper_regex a nfa1)

let regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  let something = epsilon in helper_regex regexp something
